...
public class IntervalEvalutation {
	...
	public static int getScore(NoteNode nn, Song song)
	{
		...
		if(nn.getIndex()>FingerTree.start_index)
		{
			 ...
			 */
			int oldScore = score;
			if(song.getTone(nn.getIndex()-1)<song.getTone(nn.getIndex()) && finger1>finger2)
			{
				if(!Song.isBlack(song.getTone(nn.getIndex()-1)) && nn.getParent().getFinger() != 1 && Song.isBlack(song.getTone(nn.getIndex())) && nn.getFinger() ==1)
					score+=3;
				else if (Song.isBlack(nn.getIndex()) == Song.isBlack(nn.getIndex()-1))
					score++;
			}
			else if(song.getTone(nn.getIndex()-1)>song.getTone(nn.getIndex()) && finger1<finger2)
			{
				if(!Song.isBlack(song.getTone(nn.getIndex())) && nn.getParent().getFinger() == 1 && Song.isBlack(song.getTone(nn.getIndex()-1)) && nn.getFinger() !=1)
					score+=3;
				else if(Song.isBlack(nn.getIndex()) == Song.isBlack(nn.getIndex()-1))
					score++;
			}
